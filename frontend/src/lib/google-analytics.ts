...
export class GoogleAnalyticsService {
  ...
  async connect(config: GoogleAnalyticsConfig, accessToken: string): Promise<boolean> {
    try {
      ...
      if (!config.accountId || !config.propertyId || !config.measurementId || !accessToken) {
        throw new Error('Missing required Google Analytics configuration or access token');
      }

      // Test the connection by making a real API call to validate credentials
      const isValid = await this.validateCredentials(config, accessToken);
      ...
      if (isValid) {
        ...
        if (typeof window !== 'undefined') {
          ...
          localStorage.setItem('google_analytics_token', accessToken);
        }
